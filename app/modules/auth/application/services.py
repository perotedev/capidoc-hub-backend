...
class AuthService:
    ...
    async def request_password_reset(self, email: str) -> None:
        """Enforces the one-request-per-minute cooldown for any email (registered
        or not), but only actually generates/sends a code for a real account —
        this way the rate-limit response never reveals whether the email exists."""
        if await self._password_reset_repository.is_on_cooldown(email):
            raise TooManyRequestsError("Please wait before requesting another code")

        code = await self._password_reset_repository.create_code(email)

        user = await self._user_service.get_by_email(email)
        if user is None:
            return
        await self._email_service.send_password_reset_code(user.email, code)

    async def verify_reset_code(self, email: str, code: str) -> None:
        valid = await self._password_reset_repository.verify_code(email, code)
        if not valid:
            raise UnauthorizedError("Invalid or expired code")

    async def reset_password(self, email: str, code: str, new_password: str) -> None:
        consumed = await self._password_reset_repository.consume_code(email, code)
        if not consumed:
            raise UnauthorizedError("Invalid or expired code")
        ...
        await self._user_service.change_password(user.id, new_password)
        await self._session_repository.delete_all_sessions(user.id)
