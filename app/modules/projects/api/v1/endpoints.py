...
async def search_projects(
    ...
    summaries = await service.search(query)
    return [ProjectResponse.from_summary(summary) for summary in summaries]


@router.get("/{project_id}", response_model=ProjectResponse)
async def get_project(
    project_id: UUID, _current_user: CurrentUser, service: ProjectServiceDep
) -> ProjectResponse:
...
async def create_project(
    request: ProjectCreateRequest, _current_user: CurrentUser, service: ProjectServiceDep
) -> ProjectResponse:
...
async def update_project(
    ...
    await service.update_project(project_id, request)
    summary = await service.get_project_summary(project_id)
    return ProjectResponse.from_summary(summary)


@router.delete("/{project_id}", status_code=204)
async def delete_project(project_id: UUID, _current_user: CurrentUser, service: ProjectServiceDep) -> None:
